...
class App extends React.Component {
  ...
  savePlaylist() {
    ...
    !this.state.playlistName ?
      Spotify.savePlaylist("New Playlist", tracksUri) :
  ...
  greetings() {
    ...
    if (hour < 12) {
      return this.setState({ greetings: "Good morning" })
    }
    if (hour < 18) {
      return this.setState({ greetings: "Good afternoon" })
    }
    if (hour < 22) {
      return this.setState({ greetings: "Good evening" })
    }
    if (hour < 24) {
      return this.setState({ greetings: "Good night" })
    }
  }
